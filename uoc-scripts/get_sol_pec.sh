...
then
	...
	then
		...
		cd $PEC_PATH
		if [ ! -d "$PEC_PATH/prog_datasci_sol_${input}" ]; then
	    			
			if [ "$input2" -eq "0" ]; then
				echo "Downloading PEC $input in CATALAN"
				git clone https://eimtgit.uoc.edu/prog_datasci_cat/prog_datasci_sol_${input}.git
			else
				echo "Downloading PEC $input in SPANISH"
				git clone https://eimtgit.uoc.edu/prog_datasci/prog_datasci_sol_${input}.git
			fi

			echo "OK! Files downloaded in $PEC_PATH/prog_datasci_sol_${input}"
		else
			echo "Error: the folder $PEC_PATH/prog_datasci_sol_${input} is already created."
		fi
    else
